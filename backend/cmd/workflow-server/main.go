...
import (
	...
	workflowrouter "github.com/coze-dev/coze-studio/backend/api/workflow"
	"github.com/coze-dev/coze-studio/backend/application/workflowapp"
...
func startHTTPServer(deps *workflowapp.Dependencies) {
	...
	if useSSL == "1" {
		cert, err := tls.LoadX509KeyPair(getEnv(consts.SSLCertFile, ""), getEnv(consts.SSLKeyFile, ""))
		if err != nil {
...
func setCrashOutput() {
	f, err := os.OpenFile("./panic.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o666)
	if err != nil {
		return
	}
